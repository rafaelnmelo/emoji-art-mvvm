...
//  EmojiArt
//
//  Created by Rafael Melo on 05/06/23.
//
...
class PaletteStore: ObservableObject {
    ...
    private func storeInUserDefaults() {
        defaults.set(try? JSONEncoder().encode(palettes), forKey: userDefaultsKey)
    }
    
    private func restoreFromUserDefaults() {
        if let jsonData = defaults.data(forKey: userDefaultsKey),
